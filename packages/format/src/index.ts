/**
 * 화면에 나가는 값의 표기. 모바일과 관리팀 화면이 같이 쓴다.
 *
 * 숫자는 이 파일에, **서버 코드를 사람 말로 바꾸는 표는 `labels.ts`** 에 있다.
 *
...
export { alertLevelText, alertLevelTone, slotName, type LabelTone } from './labels';
 ...
 *   "110234567890"    → "********7890"
 *
 * **지금 이 함수를 부르는 화면은 없다** (2026-09-09). 서버가 `bankAccountMasked`로
 * 가려서 주기 시작해서(회신 24번) 화면은 받은 값을 그대로 그린다.
 *
 * **서버가 가려 준 값을 여기에 다시 넣지 않는다.** 두 번 가리면 남은 네 자리까지 `*`가
 * 되어 본인도 자기 계좌인지 알아볼 수 없다. 남겨 둔 것은 `GET /employees/{id}/bank-account`
 * 처럼 **원문을 받는 경로**를 화면에 그릴 일이 생길 때를 위해서다.
 *
