 ...
 * 어디에도 쌓이지 않는다.
 *
 * **빠진 사람은 `danger`다.** 빨강의 기준은 「여기서 고칠 수 있는가」인데
...
export function SyncSecomNotice({ result }: { result: PersonSyncResult }) {
  ...
  return (
    <ResultNotice
      ...
      // 값이 안 바뀐 것을 실패로 읽지 않게 한다.
      note="이미 있는 값은 덮어쓰지 않아요. 세콤이 아는 것은 이름·카드번호·입사일뿐이라 사번과 부서는 비어 있어요."
    >
      {result.skipped.length > 0 && (
        <ResultList
          title={`${result.skipped.length}명은 넣지 못했어요. 이 사람들은 태그를 찍어도 근태가 쌓이지 않으니 직접 등록해주세요.`}
          tone="danger"
        >
