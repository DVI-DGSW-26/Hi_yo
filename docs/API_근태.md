...
## 8. 회사가 지금 쓰는 근태 자료 (2026-09-02)

관리팀이 쓰는 엑셀 두 시트를 받아 규칙만 옮겨 적는다 — 「근태일일자료」와 「근태현황」이다.
**원본은 리포에 넣지 않았다.** 실명과 7월 한 달치 실제 근태가 들어 있다 (`CLAUDE.md` 2장).

### 지각·조퇴·외출을 직군에 따라 다르게 다룬다

시트 머리에 이렇게 적혀 있다. **우리 문서에 없던 규칙이다.**

| 직군 | 지각 · 외출 · 조퇴 | 결근 |
|---|---|---|
| **관리직** | **표시만 한다** | 공제 |
| **생산직(시급)** | **공제한다. 30분 단위** | 공제 |

**앱이 판정하지 않는다.** 서버가 계산한 값을 받아 표시만 하는 원칙은 그대로다
(명세서 7.2). 다만 화면이 「지각 13분」을 보여줄 때 그것이 **급여에서 깎이는 값인지
아닌지가 직군에 따라 다르다**는 뜻이라, 급여 화면과 근태 화면의 문구를 정할 때 필요하다.

**서버가 이 구분을 하고 있는지 확인해야 한다** — `Job` 에 `payrollTarget`·`hourlyWage`가
있으니 시급직 판별은 되지만, 지각·조퇴·외출을 직군에 따라 다르게 공제하는지는 스펙에 없다.

### 주 52시간 집계

- **연장 주 12시간, 2021년 7월 시행, 주말 포함**
- 시트가 ERP 값을 옆에 두고 대조하고 있다 —
  `정상 = 평일정상 + 휴일정상 − 지각·조퇴`, **40시간 초과분은 O/T로 넘긴다.**
  `O/T = 정상 초과분 + 평일 O/T`
- 주 단위로 1~5주를 세로로 쌓고 누계를 낸다. 주간 연차·반차도 주별로 센다

### 일일자료가 담고 있는 것 (세콤 원본)

우리가 `POST /secom/ingest`로 알고 있던 것의 실제 모양이다. 23열이다.

```
근무일자 · 조직 · 이름 · 근무조 · 근무스케줄 · 출근시간 · 퇴근시간 ·
출근판정 · 퇴근판정 · 지각시간 · 외출시간 · 조기출근시간 · 연장근무시간 ·
야간근무시간 · 휴일근무시간 · 실제근무시간 · 총근무시간 · 정상근무시간 ·
외출여부 · 외출List · 복귀List · 출근 모드 · 퇴근 모드
```

- **판정이 원본에 이미 들어 있다** — `출근판정`(정상출근 등) · `퇴근판정`(연장근무 등)
- **외출을 여러 번 하면 `외출List`·`복귀List`에 시각이 쌓인다.** 우리 `AttendanceDaily`는
  `외출 시간 합계`(분) 하나만 받는다 — 몇 번 나갔는지는 화면에서 알 수 없다
- **출퇴근 모드가 `지문`이다.** 세콤 태그가 아니라 지문 인식기로 찍는다
- `조기출근시간`·`총근무시간`은 우리 스키마에 대응이 없다

### 월간 현황이 세는 것

`연차 · 지각 · 조퇴 · 외출 · 결근 · 일직 · 특근`을 **일 단위**로, `시급 기본 · 지각/조퇴/외출 ·
평일 연장 · 야간 · 특근(토·일·공휴일) · 시급 주휴`를 **시간 단위**로 센다.
날짜 칸에는 그 날 무엇이었는지(`연차` 등)를 적는다 — A-501이 하루를 가로로 보는 것과
같은 모양이다.
