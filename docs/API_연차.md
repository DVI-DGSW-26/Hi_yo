...
| 10 | **단체연차 부족분 처리가 명세서와 다르다** | 명세서 5장 예외 2번은 "부족분은 무급 처리로 자동 전환" 이라는데 **서버는 자동 전환을 하지 않고 명단으로 돌려준다** (8장). 어느 쪽이 맞는지 확정이 필요하다 |
| 11 | **A-301 화면 상세 스펙** | 연차관리대장·발생 등록 화면의 필드·정렬·버튼 동작이 정의돼 있지 않다 |
