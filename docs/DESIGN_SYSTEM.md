...
| 화면 | 1곳 | 2곳 |
|---|---|---|
| 근태 현황 | **없음** — 조회만 하는 화면이다 (달 넘기기는 secondary) | **없음** |
...
**이 표는 화면을 만들기 전에 쓴 예시다.** 근태 현황은 게이지를 쓸 자리였는데 서버가
비율을 주지 않아 접었고, 컴포넌트도 지웠다 (2026-09-03). 마이페이지 줄도 실제와 다르다 —
근속일수를 그리지 않고, 수정은 서버에 걸려 있어 버튼이 없다. **예산 규칙(최대 2곳)이
이 표의 요지이고, 칸의 내용은 지금 화면과 다를 수 있다.**
