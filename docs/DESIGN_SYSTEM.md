...
| **달력의 점** | `Calendar` `dot` — `half`만 회색이고 나머지는 그린이다 |
| `확정·완료·승인` 상태 글자 | `StatusText` `tone="done"` |
...
| 홈 | 없음 | 0 |
| S-101 마이페이지 | 없음 | 0 |
| **S-301 연차** | 선택된 날짜 · 달력 점 · `신청하기` · 신청 목록의 `승인했어요` | **4 — 넘친다** |
| S-302 연차사용계획서 | 선택된 날짜 · 달력 점 | 2 |
| S-401 재직증명서 신청 | `발급받기` | 1 |
| S-401 발급 상세 | 없음 (`PDF로 받기`는 secondary) | 0 |
| S-501 내 근태 | 없음 | 0 |
| S-503 당직 | 달력 점 · 보낸 부탁의 `바꿨어요` | 2 |
| S-503 교체 응답 | `바꿔주기` | 1 |
...
**S-301이 넘치는 것은 아직 안 고쳤다.** 무엇을 회색으로 내릴지가 화면이 무엇을 먼저
읽히게 할지의 문제라, 정하지 않고 적어 뒀다 (`00_문서_인덱스.md` 「그린 예산을 세어봤다」).
