...
| 3 | **오류 응답 미문서화** | 401/403/404/422가 스펙에 선언돼 있지 않다. **400은 요청 형식, 422는 업무 규칙 위반이라는 구분은 맞다고 확인됐다** (2026-08-31). 어떤 규칙에 422가 오는지 목록은 서버가 만들어 주기로 했다 |
| 4 | ~~명세서 발송~~ | **풀렸다 (2026-09-02).** 경로 셋이 있었다 — `GET /payroll/{id}/pdf`(확정된 것만, 아니면 422) · `POST /payroll/{id}/deliveries`(교부 기록) · `GET /payroll/{id}/deliveries`(이력). **이메일·문자 자동 발송은 범위 밖이다** — 이 API 는 관리팀이 실제로 교부한 사실을 남기는 것이고, 임금명세서 교부가 사용자의 의무라 「언제 누가 어떤 방법으로 줬는지」를 사후에 댈 수 있어야 해서다. **내려받는 것만으로는 교부 기록이 생기지 않는다.** 재교부도 사실이라 덮어쓰지 않고 쌓인다. A-602 를 만들었다 (2026-09-08, 급여 상세의 「명세서」 자리) |
| 5 | **페이지네이션 없음** | 급여대장·명세서 목록이 배열을 통째로 준다. 손봐야 한다는 데 서버도 동의했고, 지금 인원에서는 괜찮다는 판단도 같다 (2026-08-31). **`size`가 100에서 깎이는 것은 스프링 기본 동작이고 상한은 의도된 것이다** |
...
| 9 | **CORS 오리진** | `http://localhost:5173` 하나만 허용돼 있다. **주소만 주면 바로 넣어주기로 했다** — 환경변수라 재배포도 필요 없다 (2026-08-31). 관리팀 화면과 모바일 주소가 다르면 둘 다 줘야 한다 |
