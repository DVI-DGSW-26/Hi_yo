...
**화면을 만들었다** (2026-09-08). 내 신청 목록에서 `applicantSigned`가 `false`인 줄에만
「서명이 빠졌어요」가 붙고, 눌러서 들어간다 (`app/leave/sign/[requestId].tsx`).
**「신청하기」로 읽히지 않게 적었다** — 차감이 이미 끝나 있어서, 문구가 그 사실을 숨기면
직원이 아직 안 쉬어도 되는 줄 안다. `CLICK`은 쓰지 않는다(신청서와 같은 이유).
