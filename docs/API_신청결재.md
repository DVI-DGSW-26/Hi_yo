...
| # | 항목 | 왜 문제인가 |
|---|---|---|
| 1 | **전자서명 형식** | `signatureMethod`가 필수인데 `IMAGE`일 때 `signatureImage`에 무엇을 넣는지 정의가 없다 — base64인가 data URL인가, 크기 · 형식 제한은, `CLICK`일 때 이미지를 비워도 되는가. **A-302 승인 화면 전체가 여기 걸려 있다** |
...
| 5 | **결재 단계** | 개발 서버 계정에 `3 = 관리팀(결재 2단계 확인용)`이 있다. 그런데 API에 단계 개념이 보이지 않는다 — 2단계 결재가 있는가 |
...
1번과 2번이 A-302를 막고 있다. 나머지는 화면을 만들면서 채울 수 있다.
