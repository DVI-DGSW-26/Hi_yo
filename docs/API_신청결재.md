...
| 10 | **신청서에 신청인 서명 자리가 없다** | `LeaveRequestCreateRequest`는 `typeCode`·`startDate`·`endDate`·`startTime`·`endTime`·`reason`·`emergencyContact` 일곱 개뿐이다. **서명은 결재(`RequestDecisionRequest`)에만 있다** — 응답의 `signed`도 결재자 서명 여부다. 회사 「휴가(근태)신청서」 양식에 신청인 서명란이 있으면 **폰으로 신청을 끝낼 수 없다** (2026-09-02 확인) |
