...
**전체 교체다. 안 실은 값은 지워진다** (2026-09-09 서버 정정).

9-09 오전에 「`residentNo`는 보존된다」를 「전부 보존된다」로 잘못 옮겨 적었고, 그 전제로
A-102 저장을 만들었다. 서버가 코드를 다시 확인해 정정해 줬다 — `PUT`은 **처음부터 전체
교체**였다. 이름만 담아 보내면 연락처·주소·이메일에 더해 **부서·직무까지 지워진다.**

**저장 화면은 조회로 읽은 값을 전부 다시 싣는다.** 사용자가 지운 칸만 비워 보낸다
(`toUpdateInput`, PR #138).

| 비우는 방법 | 결과 |
|---|---|
| 키 생략 · `null` · `""` · 공백만(`" "`) | 넷 다 **「지움」**이다 |
| 날짜 · `departmentId` · `jobId`에 `""` | 400이 아니라 **「지움」**이다 |
| `name` · `corporation` · `hireDate` | **비울 수 없다.** 400 |

이번 배포에서 항목마다 달랐던 빈 문자열 처리가 통일됐다.
...
**값을 「비우는」 방법도 확인됐다** (2026-09-09). 키 생략·`null`·`""`·공백이 전부
「지움」이다. 그래서 A-102는 빈 칸을 「지운다」로 다루고 화면에도 그렇게 적는다 —
연락처가 바뀐 것이 아니라 없어진 경우를 다룰 수 있다.
