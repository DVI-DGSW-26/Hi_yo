...
| 확인일 | 2026-08-28 · **후보 조회 2026-09-02 (서버 회신)** |
...
> **교체 상대 후보가 열렸다** (2026-09-02). 본인용 경로가 따로 생겨 S-503의 교체 신청을
> 만들 수 있게 됐다 — 7장 1번을 본다.
...
| GET | `/duty/schedules/{scheduleId}/swap-candidates` | **그 당직을 대신 서 줄 수 있는 사람** (2026-09-02에 열렸다) |
...
| `GET /duty/schedules/{id}/swap-candidates` | `200` | **`200`** (2026-09-02 회신) |

**권한이 제대로 걸려 있다.** 남의 일정을 id로 훑을 수 없다 — 재직증명서 단건·PDF가
열려 있는 것과 대조된다 (`docs/00_문서_인덱스.md` 「서버에 알려야 할 것」).

> **명단은 관리팀만 본다는 것이 의도였고**, 대신 본인용으로 후보만 담은 조회가 따로
> 열렸다 (2026-09-02). 명단 전체가 아니라 **그 배정을 대신 설 수 있는 사람만** 온다.
...
### `DutySwapCandidateResponse`

`employeeId` `name`

**둘뿐이다.** 부서도 순번도 오지 않는다 — 고르는 데 필요한 것만 담겨 있다.
그래서 **동명이인은 화면에서 구분되지 않는다.**
...
### ~~1. 교체 상대 후보를 본인이 볼 수 없다~~ — 풀렸다 (2026-09-02)

**`GET /duty/schedules/{scheduleId}/swap-candidates`가 열렸다.** 일반 직원이 부를 수 있고,
그 배정을 대신 설 수 있는 사람만 `employeeId`·`name`으로 온다.

8-31에 서버가 "같은 명단 사람만 담은 조회를 따로 여는 쪽이 나아 보이나 당직 운영 방식이
확정 전이라 그때 같이 정하겠다"고 했던 것이 그대로 그 모양으로 들어왔다. 명단
대상자(`/duty/rosters/{id}/members`)와 당직표 전체는 **관리팀 전용 그대로다** —
막힌 것을 연 것이 아니라 본인용 경로를 새로 낸 것이다.

**S-503의 교체 신청을 만들었다** (2026-09-07). 8-28부터 절반만 있던 화면이 마저 붙었다.

### ~~2. `dayOfWeek`의 형식~~ — 풀렸다 (2026-09-02)

**한글 한 글자다** — `"월"` … `"일"`. `SATURDAY`가 아니다. 근태표와 당직표 둘 다 같다.

앱은 여전히 `dutyDate`에서 요일을 뽑는다. 값이 `null`로 올 수 있는 필드라 날짜에서
세는 편이 빈 줄을 만들지 않는다 — 형식이 확인됐다고 바꿀 이유는 없다.
